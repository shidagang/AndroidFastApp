package com.fastapputil.widget.bottomtab;
...
import com.fastapputil.widget.bottomtab.notification.AHNotification;
import com.fastapputil.widget.bottomtab.notification.AHNotificationHelper;
...
public class AHBottomNavigation extends FrameLayout {
	...
	private OnChoosePersonalBeforeListener listener;
	...
	private void createClassicItems(LinearLayout linearLayout) {
		...
		float inactiveSize = resources.getDimension(R.dimen.bottom_navigation_text_size_active);
...
//		} else if (forceTitlesDisplay && items.size() > MIN_ITEMS) {
			...
			inactiveSize = resources.getDimension(R.dimen.bottom_navigation_text_size_forced_active);
//		}
...
//					current ? itemActiveColor : itemInactiveColor, forceTint));
			// TODO placeholder
			Glide.with(context)
					.load(current ? item.getImgSelectedUrl() : item.getImgNormalUrl())
					.diskCacheStrategy(DiskCacheStrategy.RESULT)
					.dontAnimate()
					.into(icon);
			...
			view.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					if(itemIndex != items.size() - 1 || listener == null || listener.onChoosePersonalBefore()) {
...
//					currentItem == i ? itemActiveColor : itemInactiveColor, forceTint));
			Glide.with(context)
					.load(currentItem == i ? item.getImgSelectedUrl() : item.getImgNormalUrl())
					.diskCacheStrategy(DiskCacheStrategy.RESULT)
					.dontAnimate()
					.into(icon);
...
//				AHHelper.updateDrawableColor(context, items.get(itemIndex).getImgSelectedUrl(), icon,
//						itemInactiveColor, itemActiveColor, forceTint);
//				AHHelper.updateDrawable(context, items.get(itemIndex).getImgSelectedUrl(), icon);
				AHHelper.updateDrawable(context, items.get(itemIndex).getImgSelectedUrl(), icon);
...
//						itemActiveColor, itemInactiveColor, forceTint);
				AHHelper.updateDrawable(context, items.get(currentItem).getImgNormalUrl(), icon);
...
//						itemActiveColor, itemInactiveColor, forceTint);
	...
	public void setOnChoosePersonalBeforeListener(OnChoosePersonalBeforeListener listener) {
		this.listener = listener;
	}


	public interface OnChoosePersonalBeforeListener {
		boolean onChoosePersonalBefore();
	}
	////////////////
	// INTERFACES //
	////////////////
